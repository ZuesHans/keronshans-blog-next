...
- **做题见到的结论**
  -` a + b = (a OR b) + (a AND b) `这是因为 OR 相当于"有进位地加"，AND 捕获了所有进位，两者加起来恰好等于 a+b。
  - 任何偶数 $2k$ 与相邻的奇数 $2k+1$ 异或，结果永远是 $1$。即：$(2k) \oplus (2k+1) = 1$。
  - 基于这个本质，我们研究前缀异或和 $P_n = \bigoplus_{i=1}^n i$ 时，每 4 个数就会发生一次美妙的抵消：$$(2k) \oplus (2k+1) \oplus (2k+2) \oplus (2k+3) = 1 \oplus 1 = 0$$这就推导出了那个无敌的 周期为 4 的分段函数：设 $n \pmod 4$ 的余数为 $m$：前缀异或的结果为：
    - m==0 ->n
    - m==1 -> 1
    - m==2 -> n+1
    - m==3 -> 0
    - 求任意区间 $[L, R]$ 的异或和：
利用前缀和思想，直接 $O(1)$ 求解：$\bigoplus_{i=L}^R i = P_R \oplus P_{L-1}$。**前缀和公式（x+i）/4  （x是前缀到几，i是你要的余数）**
...
- **结论集合**
  ...
  - 常用技巧
    判断2的幂：a & (a-1) == 0
    去掉最低位1：a & (a-1)
    (a + b) / 2 = (a & b) + (a XOR b) / 2（无溢出求平均）
  - GCD (最大公约数) 的差分性质 (辗转相减法衍生)$$\gcd(a, b) = \gcd(a, b-a)$$
