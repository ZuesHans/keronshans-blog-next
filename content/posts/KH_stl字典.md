...
- 在状态空间搜索、图的连通性、二维偏序或 01 背包问题中，当状态数 $N$ 在 1000~2000 级别，常规 $O(N^2)$ 或 $O(N^3)$ 做法会 TLE 时，使用 `std::bitset` 可以利用 CPU 的位运算指令将常数极大地优化（通常是将时间复杂度除以 64），实现“降维打击”。
...
// 声明一个长度为 1000 的 bitset，默认所有位都是 0 (灭)
bitset<1000> bs; 

// 声明并用整数初始化 (十进制转二进制，5 -> 101，即第 0 和第 2 位为 1)
bitset<1000> bs2(5);
```
...
```cpp
bs1 |= bs2 //(按位或，合并集合)：bs2 中为 1 的位，全部在 bs1 中设为 1。常用于状态转移、图的连通性。

bs1 &= bs2 //(按位与，求交集)：bs1 和 bs2 同一个位置都为 1 时才保留为 1，其他全为 0。

bs1 ^= bs2 //(按位异或，找不同)：两个位置状态相同为 0，不同为 1。常用于博弈论、奇偶校验。

bs <<= k //(整体左移)：所有状态整体左移 k 位，右侧补 0。01 背包求可行性时的核武器。
```

- 常用查询统计

```cpp
bs.count()//统计当前共有多少位是 1。底层指令极快，求集合元素个数必备。

bs.any()//只要有任意一位是 1，就返回 true。

bs.none()//如果所有位都是 0，就返回 true。
...
```cpp
当需要遍历 bitset 里所有为 1 的位（比如找寻到达了哪些顶点）时，不要用普通 for 循环逐个 test()，使用底层优化的内置函数能快几十倍：

bs._Find_first()//返回第一个为 1 的位的下标。如果全为 0，返回 bitset 的大小（如 1000）。

bs._Find_next(i)//返回在第 i 位之后，下一个为 1 的位的下标。
